  ...
  async syncAllProfilePhotos(): Promise<{ success: number; failed: number }> {
    ...
    const results = await Promise.allSettled(
      students.map(async (student) => {
        try {
          // Use retry logic for profile photo fetch
          const stats = await retryWithBackoff(
            () => this.fetchUserStats(student.leetcodeUsername),
            { maxAttempts: 3, initialDelayMs: 1000 }
          );

          if (stats?.profilePhoto && stats.profilePhoto !== student.profilePhoto) {
            await storage.updateStudent(student.id, { profilePhoto: stats.profilePhoto });
            console.log(`✅ Profile photo updated for ${student.leetcodeUsername}`);
            return true;
          }
          return true; // No update needed, but not a failure
        } catch (error) {
          console.error(`❌ Failed to update profile photo for ${student.leetcodeUsername} after retries:`, error);
          return false;
        }
      })
    );
